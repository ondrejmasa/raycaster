...
namespace pf
{
    ...
    inline bool isValid(const std::vector<std::vector<int>>& grid, int row, int col)
    {
        return (row >= 0) && (row < grid.size()) && (col >= 0) and (col < grid[0].size());
    }

    inline bool isUnBlocked(const std::vector<std::vector<int>>& grid, const int row, const int col)
    {
        return (grid[row][col] == 0);
    }

    inline bool isDestination(const int row, const int col, const Pair& dest)
    {
        return (row == dest.first and col == dest.second);
	}

    inline double calculateHValue(const int row, const int col, const Pair& dest)
    {
        return ((double)sqrt((row - dest.first) * (row - dest.first) + (col - dest.second) * (col - dest.second)));
	}

    inline std::vector<Pair> tracePath(const std::vector<std::vector<cell>>& cellDetails, const Pair& dest)
    {
    int row = dest.first;
    int col = dest.second;
    ...
    inline std::vector<Pair> aStarSearch(const std::vector<std::vector<int>>& grid, const Pair& src, const Pair& dest)
    {
        ...
        int i = src.first; int j = src.second;
